...
if (isset($_POST['vbtn'])) {
    if ($_SESSION['role'] == 'admin') {
        $inf = $pdo->prepare("SELECT *, SUM(MONTANT_COTISATION) as total, adherent.sexe_adherent as classe FROM  cotisation_mensuelle join adherent on adherent.id_adherent = cotisation_mensuelle.adherent  where ANNEE_COTISATION = ? AND adherent.commune = ? group by MOIS_COTISATION, classe order by MOIS_COTISATION ASC");
        $inf->execute([$_POST['annee'], $_POST['commune']]);
    } else {
    ...
    if (isset($_POST['annee'])) {
        $etat = fopen("etat_cotisations_mensuelles_" . $_POST['annee'] . " de " . $_POST['commune']. ".xls", "w");
                                ...
                                <div class="row">
                                    <form action="" method="POST">
                                        <div class="row">
                                            ...
                                            <div class="col-2">
                                                <div class="">
                                                    <select class="form-select" id="commune" aria-label="Floating label select example" required name="commune">
                                                        <option selected value="">...</option>
                                                        <?php foreach ($communes as $commune) { ?>
                                                            <option value="<?= $commune['ID_COMMUNE'] ?>" <?php if (isset($_POST['vbtn']) && $_POST['commune'] == $commune['ID_COMMUNE']) {
                                                                                                                echo "selected";
