...
if (isset($_POST['vbtn'])) {
    if ($_SESSION['role'] == 'admin') {
        $req = $pdo->prepare("SELECT * from adherent join commune on adherent.commune = commune.ID_COMMUNE where id_adherent not in (select adherent from deces) AND adherent.commune = ?");
        $req->execute([$_POST['commune']]);
    } else {
                    ...
                    <div class="card-body">
                        ...
                        <form action="" method="POST">
                            <div class="row">
                                <div class="col-4">
                                    <div class="">
                                        <select class="form-select" id="annee" aria-label="Floating label select example" required name="commune">
                                            <option selected value="">...</option>
                                            <?php foreach ($communes as $commune) { ?>
                                                <option value="<?= $commune['ID_COMMUNE'] ?>" <?php if (isset($_POST['vbtn']) && $_POST['commune'] == $i) {
                                                                                                    echo "selected";
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="table-responsive">
                                    <?php if (isset($_POST['commune']) && $data !== array()) { ?>
                                        <table class="table">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                        ...
                        <li><a class=\"dropdown-item cotisationDecesBtn\" data-bs-toggle=modal data-bs-target=#cotisationDecesModal data-to-send=\"$v[id_adherent],$v[nom_adherent] $v[prenom_adherent]\" data-bs-target=#staticBackdrop2>Cotisations pour les décès</a></li>
                                    ...
                                    <?php } else { ?>
                                        <h3>Aucunes informations disponible pour cette commune</h3>
                                    <?php } ?>
                                </div>
                            </div>
                        </div>
